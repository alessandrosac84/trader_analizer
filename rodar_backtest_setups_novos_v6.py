"""
rodar_backtest_setups_novos_v6.py — Rodada 6 em UM comando.
...
def main():
    ...
    from backtest_setups_novos_v6 import STRATS
    n_b3, n_cy = len(STRATS["b3"]), len(STRATS["crypto"])

    print(f"{'#' * 70}\n# BACKTEST SETUPS NOVOS V6 — {datetime.now():%d/%m %H:%M}"
          f"\n# B3: {n_b3} setups · Crypto: {n_cy} setups"
          f"\n# Grupos: {grupos} · bars={args.bars}"
          f"\n# Foco: refinar 🟡 + frequência com qualidade"
    ...
    for g in grupos:
        ...
        r = subprocess.run(
            [sys.executable, str(ROOT / "backtest_setups_novos_v6.py"),
    ...
    for g, (st, dur) in resultados.items():
        print(f"  {g:<8} {st}  ({dur})")
    print("\n# Logs: logs/backtest_setups_novos_v6_<grupo>_*.txt"
