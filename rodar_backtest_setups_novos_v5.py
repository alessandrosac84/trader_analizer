"""
rodar_backtest_setups_novos_v5.py — Rodada 5 em UM comando.
...
def main():
    ...
    from backtest_setups_novos_v5 import STRATS
    n_b3, n_cy = len(STRATS["b3"]), len(STRATS["crypto"])

    print(f"{'#' * 70}\n# BACKTEST SETUPS NOVOS V5 — {datetime.now():%d/%m %H:%M}"
    ...
    for g in grupos:
        ...
        r = subprocess.run(
            [sys.executable, str(ROOT / "backtest_setups_novos_v5.py"),
    ...
    for g, (st, dur) in resultados.items():
        print(f"  {g:<8} {st}  ({dur})")
    print("\n# Logs: logs/backtest_setups_novos_v5_<grupo>_*.txt"
