...
# Rede de segurança final (14/08): corte DURO INCONDICIONAL — não depende do micro.
# O loss-cut (regra 7) só age com o micro contra; a maioria das perdas de agosto
# foi "moer até o −1R" sem o micro virar. Este corte fecha em −0,8R sempre, para
...
def manage(pos: dict, signal: dict, atr: float = 0.0, candles_open: int = 0,
    ...
    try:
        ...
        # 8) REDE DE SEGURANÇA FINAL — CORTE DURO INCONDICIONAL (−0,8R)
        # Backstop: só age se nada acima já fechou. Não depende do micro nem de
        # pico. Capa a perda em −0,8R antes de virar stop cheio (−1R). Se o loss-cut
        # (regra 7) já agiu em −0,7R com micro, este nem chega a ser avaliado.
        if (HARD_CUT_ENABLED and not close and risk > 0
                and favor <= -HARD_CUT_R * risk):
            rec, reason, close = (
                "FECHAR",
                f"🛑 HARD-CUT −{HARD_CUT_R:.1f}R (incondicional) — corta antes do stop cheio.",
                True,
            )
            close_code = "HARD_CUT"
            logger.info(
                "HARD_CUT %s: favor=%.2f risk=%.2f r_now=%.2f pnl=%.2f (backstop incondicional)",
                pos.get("symbol") or "?", favor, risk,
                favor / risk if risk else 0.0, cur_pnl,
            )
