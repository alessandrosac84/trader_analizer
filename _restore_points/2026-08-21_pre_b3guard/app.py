...
# ── OPENING ENGINE (abertura do WIN — tela de análise, NÃO envia ordem) ─────
# Aditivo e isolado: página /opening, endpoints /api/opening/*. Score
# determinístico 0–100. Não altera nenhum motor existente.
try:
    from blueprints.opening_bp import opening_bp
    app.register_blueprint(opening_bp)
except Exception as _op_err:
    import logging as _lgop
    _lgop.getLogger(__name__).warning("opening_bp não registrado: %s", _op_err)

# ── MONITOR PROFIT (score v6 + fluxo do Profit — análise, NÃO envia ordem) ──
# Aditivo e isolado: página /profit-monitor. Não altera Monitor MT5, V7 nem Crypto.
try:
    from blueprints.profit_monitor_bp import profit_monitor_bp
    app.register_blueprint(profit_monitor_bp)
except Exception as _pm_err:
    import logging as _lgpm
    _lgpm.getLogger(__name__).warning("profit_monitor_bp não registrado: %s", _pm_err)
...
def _startup_once():
    ...
    try:
        # OPENING ENGINE (análise da abertura do WIN) — thread própria, sem ordem.
