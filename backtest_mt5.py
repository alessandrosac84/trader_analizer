...
def run(symbol, tf_key, bars, forward, min_score, rr_override=None):
    ...
    for i in range(lookback, len(df) - forward):
        ...
        htf_window = None
        if htf is not None:
            htf_window = htf[htf.index <= df.index[i]]
        ...
        if None in (e, s, t):
            continue
        # --rr: sobrescreve o alvo por um múltiplo fixo do risco (testa exit sem S/R)
        if rr_override:
            risk = abs(e - s)
            t = (e + rr_override * risk) if sig["acao"] == "COMPRA" else (e - rr_override * risk)
        outcome, R = _evaluate(sig["acao"], e, s, t, future)
        results.append({
            "acao": sig["acao"], "score": sig["score"],
...
def _report(symbol, r, min_score):
    st = _stats(r)
...
if __name__ == "__main__":
    ...
    summary = []
    for sym in symbols:
        out = run(sym, args.tf, args.bars, args.forward, _sym_min(sym), args.rr)
        if out:
            st = _stats(out["df"]); st["symbol"] = out["symbol"]
            summary.append(st)
    if summary:
        print("\n" + "═" * 56)
        print("  RESUMO (o que importa é a EXPECTÂNCIA, não o win rate)")
        for st in summary:
